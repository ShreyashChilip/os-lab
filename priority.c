...
void sortProcesses(int n, int processes[], int bt[], int at[], int priority[]) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
...
int main() {
    ...
    int processes[n], bt[n], wt[n], tat[n], at[n], priority[n];
    ...
    sortProcesses(n, processes, bt, at, priority);
